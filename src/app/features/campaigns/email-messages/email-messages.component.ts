      ...
      <mat-card-content>
        <mat-tab-group [(selectedIndex)]="activeTab" (selectedIndexChange)="onTabChange($event)">
          ...
          <mat-tab [label]="'EMAIL_MESSAGES.CONFIG_TAB' | translate">
            <div class="tab-content">
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>SMTP Configuration (JSON)</mat-label>
...
export class EmailMessagesComponent implements OnInit {
  ...
  createMessage(): void {
    ...
    this.defaultService.postEmail(JSON.stringify(body)).subscribe({
      next: () => {
        this.snackBar.open('EMAIL_MESSAGES.SUCCESS', undefined, { duration: 3000 });
        this.newTo = '';
        this.newSubject = '';
        this.newBody = '';
  ...
  saveConfig(): void {
    ...
    this.defaultService.putEmailConfiguration(parsed).subscribe({
      next: () => {
        this.snackBar.open('EMAIL_MESSAGES.SUCCESS', undefined, { duration: 3000 });
      },
    });
