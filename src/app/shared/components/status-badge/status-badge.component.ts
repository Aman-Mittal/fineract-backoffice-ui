...
export class StatusBadgeComponent {
  ...
  get colorClass(): string {
    ...
    const code = typeof this.status === 'string' ? this.status.toLowerCase() : ((this.status['code'] as string)?.toLowerCase() || '');
    const value = typeof this.status === 'string' ? this.status.toLowerCase() : ((this.status['value'] as string)?.toLowerCase() || '');
    ...
    if (code.includes('closed') || value.includes('closed') || code.includes('rejected') || value.includes('rejected') || code.includes('deleted')) {
